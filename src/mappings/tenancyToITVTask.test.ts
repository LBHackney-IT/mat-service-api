...
describe('tenancyToITVTask', () => {
  it('returns a valid ITV task when given a Tenancy', () => {
    ...
    expect(ITVTask.serviceRequest.description).toEqual(
      'Starting an initial tenancy visit'
    );
    expect(ITVTask.serviceRequest.contactId).toEqual(
      tenancy.contacts[0].contactId
    );
    expect(ITVTask.serviceRequest.subject).toEqual(
      'c1f72d01-28dc-e711-8115-70106faa6a11'
    );
  });
});
