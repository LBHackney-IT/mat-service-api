...
describe('createITVTasks', () => {
  it('should retrieve new tenancies and create ITV tasks for them', async () => {
    ...
    const createTenancyManagementInteraction = jest.fn(
      async (tmi: TenancyManagementInteraction) => ({
        body: [],
        error: undefined,
      })
    );
    ...
    const v1MatAPIGateway = {
      ...
      createTenancyManagementInteraction: <jest.Mock>(
    ...
    expect(createTenancyManagementInteraction.mock.calls[0][0]).toEqual(
      tenancyToITVTask(tenancies[0])
    );
    expect(createTenancyManagementInteraction.mock.calls[1][0]).toEqual(
      tenancyToITVTask(tenancies[1])
    );
  });
