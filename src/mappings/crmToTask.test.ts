...
describe("crmResponseToTask", () => {
  it("returns a valid task when given a crmResponse", () => {
    ...
    convertedTask.forEach(task => {
      ...
      expect(task.resident.presentationName).toEqual(crmValue["contact1_x002e_fullname"])
      expect(task.resident.role).toEqual("Primary Tenant")
      ...
      expect(task.dueTime).toEqual(crmValue["hackney_completiondate"])
