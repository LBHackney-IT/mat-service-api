...
describe("crmToPatchDetails", () => {
    it("returns a valid patch details when given an crm response", () => {
        ...
        expect(convertedPatchDetails.officername).toEqual(crmResponse.value[0]["_hackney_patchid_value@OData.Community.Display.V1.FormattedValue"])
        expect(convertedPatchDetails.patchid).toEqual(crmResponse.value[0].hackney_estateofficerpatchid)
        expect(convertedPatchDetails.patchname).toEqual(crmResponse.value[0].hackney_name)
    })
})